trigger VOD_MEDICAL_INQUIRY_AFTER_INSERT_UPDATE on Medical_Inquiry_vod__c (after insert, after update, after delete) {
    ...
    if (Trigger.isDelete){
        ...
        if (groupsToUpdateCount.size() > 0){
            List<Medical_Inquiry_vod__c> siblingsMI = [SELECT Id, Group_Identifier_vod__c FROM Medical_Inquiry_vod__c WHERE group_identifier_vod__c IN :groupsToUpdateCount];
